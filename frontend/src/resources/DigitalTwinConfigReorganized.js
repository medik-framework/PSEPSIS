export const Cardiovascular = {
  ...
  measurements: {
    ...
    CardioOutput: {
      ...
      minValue: 0.0, maxValue: 20, decimal: 1,
  ...
  assessments: {
    Hypotensive: {
      name: "Hypotensive", fulltext: "Hypotensive",
      type: "boolean",
      formula: (patient, organs, _) => {
        ...
        if ([4,5,6,7].includes(AgeGroupVitals) && BPSys < 70+AgeInYears*2) return true;
    ...
    CardioDysfunction: {
      name: "Cardio Dysfunction Score", fulltext: "Cardiovascular Dysfunction Score",
      type: "number", minValue: 0, maxValue: 1, decimal: 0,
      formula: (patient, organs, treatments) => {
        let mapScore, perfusionScore, vasoScore = 0;
        let uoScore, lactateScore, capScore, metabolicAcidosisScore, tempGradientScore = 0; 
        ...
        if (organs.Cardiovascular.measurements.BP.MAP.value < MAPThres) mapScore = 1;
        if (organs.Renal.measurements.UrineOutput.value < 0.5) uoScore = 1;
        if (organs.Cardiovascular.measurements.Lactate.value >= 5) lactateScore = 1;
        if (organs.Cardiovascular.measurements.CapillaryRefill.value == 5) capScore = 1;
        if (organs.Renal.measurements.baseExcess.value < -5) metabolicAcidosisScore = 1;
        if (Math.abs(organs.Immune.measurements.CoreTemp.value - organs.Immune.measurements.Temp.value) > 3) tempGradientScore = 1;
        if ((capScore + lactateScore + uoScore + metabolicAcidosisScore + tempGradientScore) >= 2) perfusionScore = 1;
        if (treatments.Drug.Norepinephrine.totalDosage > 0 || 
...
export const Respiratory = {
  ...
  assessments: {
    ...
    RespDysfunction: {
      name: "Respiratory Dysfunction Score", fulltext: "Respiratory Dysfunction Score",
      type: "number", minValue: 0, maxValue: 1, decimal: 0,
      formula: (patient, organs, treatments) => {
        ...
        let FPRatio = organs.Respiratory.measurements.PaO2.value/organs.Respiratory.measurements.FiO2.value;
        let SFRatio = organs.Respiratory.measurements.SpO2.value/organs.Respiratory.measurements.FiO2.value;
        if (!treatments.Operation.RespiratorySupport.On){
          if ((FPRatio<300 && !patient.Comorbidity.includes("LungDisease")
            && !patient.Comorbidity.includes("CongenitalHeartDisease")) ||
            (organs.Respiratory.measurements.SpO2.value <= 97 && SFRatio < 264))
            return 1;
        } else {
          if ((FPRatio<200 && !patient.Comorbidity.includes("LungDisease")
            && !patient.Comorbidity.includes("CongenitalHeartDisease")) || 
...
export const Renal = {
  ...
  measurements : {
    pH: { 
      ...
      unit: "", type: "number",
      minValue: 0.0, maxValue: 14, decimal: 1,
  ...
  assessments: {
    ...
    RenalDysfunction: {
      name: "Renal Dysfunction Score", fulltext: "Renal Dysfunction Score",
      type: "number", minValue: 0, maxValue: 2, decimal: 0,
      formula: (patient, organs, _) => {
        let creatinineScore , baselineCreatinineScore = 0;
        const AgeGroupShock = patient.Age.AgeGroupShock;
        const CreatinineHighMap = {1:0.8 ,2:0.3, 3:0.4, 4:0.6, 5:0.7, 6:1.0, 7:1.2};
        const CreatinineThres = CreatinineHighMap[AgeGroupShock];
        if (organs.Renal.measurements.Creatinine.value >= CreatinineThres) creatinineScore = 1;
        if (organs.Renal.measurements.Creatinine.value >= 2*organs.Renal.measurements.BaselineCreatinine.value) 
  ...
  assessments: {
    HematologicDysfunction: {
      ...
      formula: (_, organs, _) => {
        ...
        if (plateletHistHigh - plateletValue >= 0.5 * plateletHistHigh) return 1;
...
export const Hepatic = {
  ...
  assessments: {
    HepaticDysfunction: {
      name: "Hepatic Dysfunction Score", fulltext: "Hepatic Dysfunction Score",
      type: "number", minValue: 0, maxValue: 1, decimal: 0,
      formula: (patient, organs, _) => {
        const newBorn = patient.Age.AgeInDays <= 28;
        let ALTHighThres;
        if (newBorn) ALTHighThres = 25;
        else if (patient.Age.AgeInYears <= 1) ALTHighThres = patient.Gender ? 35 : 30;
        else if (patient.Age.AgeInYears <= 3) ALTHighThres = 30;
        else if (patient.Age.AgeInYears <= 6) ALTHighThres = patient.Gender ? 20 : 25;
        else if (patient.Age.AgeInYears <= 9) ALTHighThres = 25;
        else if (patient.Age.AgeInYears <= 18) ALTHighThres = patient.Gender ? 30 : 20;
        const score = ((organs.Hepatic.Bilirubin.value >= 2 && !newBorn) || 
                      organs.Hepatic.ALT.value >= 2 * ALTHighThres) ? 1:0;
        return score;
      }
    }
  }
